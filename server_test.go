...
func TestServerHasCorrectInitialValues(t *testing.T) {
	assert(t, server != nil, "global instance is initialized")
...
func TestAddError(t *testing.T) {
	...
	assert(t, len(server.errors) == 201, "there are many errors (201), and are actually:", len(s.errors))
	assert(t, s.hasError(), "has error should report error but does not")
	assert(t, s.getError() != nil, "get error should not be nil")
}
...
func TestServerStopIsSafeToRunAfterCleanup(t *testing.T) {
	s := newServer()
	s.start()
	assert(t, s.isRunning(), "server should run after its started")
